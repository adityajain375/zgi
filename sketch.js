...
function setup(){
  ...
  player = createSprite(displayWidth/2+70,displayHeight/2-90,30,30);

  rand2 = random(2,5);

  topM = createSprite(displayWidth/2,15,displayWidth-100,5);

  bottom = createSprite(displayWidth/2,displayHeight-135,displayWidth-100,5);

  left = createSprite(50,displayHeight/2-60,5,displayHeight-145);

  right = createSprite(displayWidth-50,displayHeight/2-60,5,displayHeight-145);
  ...
  health = 100;
  healthBar = createSprite(player.x,player.y+40,health,5);
...
fill("yellow");
  ...
  if(gameStateMain === 0){
    ...
    text("PRESS 'W' TO SHOOT UP",displayWidth/2,displayHeight/2);
    text("PRESS 'S' TO SHOOT DOWN",displayWidth/2,displayHeight/2+30);
    text("PRESS 'A' TO SHOOT LEFT",displayWidth/2,displayHeight/2+60);
    text("PRESS 'D' TO SHOOT RIGHT",displayWidth/2,displayHeight/2+90);
  ...
  if(player.x<70 || player.x>displayWidth-70 || player.y<30 || player.y>displayHeight-150){
    player.velocityX = 0;
    player.velocityY = 0;
  }

  if(player.x<70 && keyWentDown("RIGHT_ARROW")){
    player.velocityX += 5;
  }

  if(player.x>displayWidth-70 && keyWentDown("LEFT_ARROW")){
    player.velocityX += -5;
  }

  if(player.y<30 && keyWentDown("DOWN_ARROW")){
    player.velocityY += 5;
  }

  if(player.y>displayHeight-150 && keyWentDown("UP_ARROW")){
    player.velocityY += -5;
  }
  ...
  if(bullet3.x>displayWidth-50){
    gameState3 = 0;
  }

  if(bullet4.y>displayHeight-145){
    gameState4 = 0;
  }
  ...
  if(health>75){
    fill("green");
    text(health+"% (Good)",player.x-35,player.y+60);
  }

  if(health>50 && health<75 || health===75){
    fill("yellow");
    text(health+"% (Not So Good)",player.x-55,player.y+60);
  }

  if(health>25 && health<50 || health===50){
    fill("orange");
    text(health+"% (Bad)",player.x-30,player.y+60);
  }

  if(health>0 && health<25 || health===25){
    fill("red");
    text(health+"% (Critical)",player.x-40,player.y+60);
  }
